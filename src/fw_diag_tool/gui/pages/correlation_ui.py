...
提供多協定（I2C、SPI、UART）時間線對齊視覺化，自動偵測跨協定
...
def _build_timeline_chart(events: list[dict[str, Any]]) -> go.Figure:
    ...
    for proto in ("I2C", "SPI", "UART"):
        proto_events = [e for e in events if e["protocol"] == proto]
        if not proto_events:
            continue
...
def render() -> None:
    ...
    st.caption(
        "上傳多組協定追蹤資料（I2C、SPI、UART），自動對齊時間線並偵測跨協定異常叢集，"
    ...
    render_guide_expander(
        ...
        fallback_body=(
            "### 使用場景\n\n"
            "當系統發生難以單一協定解釋的故障時（例如 I2C NACK 與 SPI 寫入失敗同時出現），"
            "跨協定時間線能幫助工程師觀察事件發生順序，判定哪個事件是因、哪個是果。\n\n"
            "### 操作步驟\n\n"
            "1. 在下方分別上傳 I2C CSV、SPI CSV 及/或 UART 日誌。\n"
