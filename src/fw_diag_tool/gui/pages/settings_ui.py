...
def get_current_settings() -> dict[str, Any]:
    ...
    i2c_timeout = float(
        st.session_state.get("settings_i2c_timeout", DEFAULT_I2C_TIMEOUT_MS)
    )
    ...
    spi_page_size = int(
        st.session_state.get("settings_spi_page_size", DEFAULT_SPI_PAGE_SIZE)
    )
...
def render() -> None:
    ...
    st.header("偏好設定（Settings & Preferences）")
    st.caption("自訂全域協定分析逾時、介面語系、視覺主題與資料載入上限。")
    ...
    i2c_timeout = st.number_input(
        ...
        step=1.0,
        format="%.1f",
        help="I2C / SMBus 交易超時判定門檻（毫秒）。",
    )

    # 2. 預設語言
    locale_opts = list(LANGUAGE_OPTIONS)
    locale_idx = (
        locale_opts.index(current["locale"])
        if current["locale"] in locale_opts
        else 0
    )
    ...
    theme_idx = (
        theme_opts.index(current["theme"])
        if current["theme"] in theme_opts
        else 0
    )
    ...
    spi_page_size = st.number_input(
        ...
        step=256,
        help="SPI NOR Flash Page Program 預設緩衝區大小（位元組）。",
    )

    # 按鈕列
    col_apply, col_reset, _ = st.columns([1, 1, 3])
    with col_apply:
        apply_label = t("btn_apply", domain="gui")
        if st.button(apply_label, type="primary", key="btn_apply_settings"):
            ...
            show_success_toast("設定已成功套用！")

    with col_reset:
        reset_label = f"{t('btn_reset', domain='gui')}為預設值"
        if st.button(reset_label, key="btn_reset_settings"):
            reset_settings()
            show_success_toast("已重設為預設設定！")

    # 生效設定摘要
    st.divider()
    st.subheader("目前生效設定摘要")
    active = get_current_settings()

    sum_col1, sum_col2, sum_col3, sum_col4, sum_col5 = st.columns(5)
    with sum_col1:
        st.metric("I2C Timeout", f"{active['i2c_timeout_ms']:.1f} ms")
    with sum_col2:
        st.metric("語言", str(active["locale"]))
    with sum_col3:
        st.metric("主題", str(active["theme"]))
    with sum_col4:
        st.metric("資料上限", f"{active['max_rows']:,} 列")
    with sum_col5:
        st.metric("SPI Page", f"{active['spi_page_size']} B")
