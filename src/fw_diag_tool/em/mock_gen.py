...
class EMMockGenerator:
    ...
    def generate_busctl_script(cls, config: EMBoardConfig) -> str:
        """Generate a runnable Bash launcher containing the standalone Python mock daemon."""
    ...
    def generate_python_mock(cls, config: EMBoardConfig) -> str:
        """Generate a standalone Python script using dbus-next to publish mock objects."""
        objects = cls._build_mock_objects(config)
        objects_repr = pprint.pformat(objects, sort_dicts=True, width=100)
        board_repr = repr(config.board_name)

        lines: list[str] = [
            "#!/usr/bin/env python3",
            '"""OpenBMC D-Bus Mock Daemon.',
            "",
            ...
            f"BOARD_NAME = {board_repr}",
            "",
            f"MOCK_OBJECTS = {objects_repr}",
            "",
            "",
            ...
            "    reply = await bus.request_name(MOCK_SERVICE)",
            "    if reply not in (RequestNameReply.PRIMARY_OWNER, RequestNameReply.ALREADY_OWNER):",
            '        raise RuntimeError(f"Failed to acquire D-Bus name {MOCK_SERVICE}: {reply}")',
            "    return bus",
            "",
            "",
            "async def _async_main() -> int:",
            "    bus = await publish_mock_objects()",
            "",
            '    print(f"[SUCCESS] Registered {len(MOCK_OBJECTS)} mock objects on {MOCK_SERVICE}")',
            ...
            '        sys.stderr.write(f"Error: Failed to initialize D-Bus mock service: {exc}\\n")',
            "        return 1",
            "",
            "",
