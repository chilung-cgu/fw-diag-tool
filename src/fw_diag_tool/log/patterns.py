...
PATTERN_LIBRARY: list[LogPattern] = [
    ...
    LogPattern(
        ...
        subsystem=Subsystem.GENERAL,
        severity=Severity.ERROR,
        regex=re.compile(r"ipmid.*:\s*(?:Timed out waiting for|command timed out|no response from host)", re.IGNORECASE),
        extract_fields=["extra"],
    ...
    LogPattern(
        ...
        subsystem=Subsystem.GENERAL,
        severity=Severity.ERROR,
        regex=re.compile(r"systemd\[\d+\]:\s*\S+\.service:\s*(?:Main process exited.*status=(?!0/SUCCESS\b)|Failed with result|entered failed state)", re.IGNORECASE),
        extract_fields=["extra"],
    ...
    LogPattern(
        ...
        subsystem=Subsystem.GENERAL,
        severity=Severity.ERROR,
        regex=re.compile(r"systemd-journald.*:\s*(?:Failed to write entry.*No space left on device|No space left on device)", re.IGNORECASE),
        extract_fields=["extra"],
    ...
    LogPattern(
        ...
        subsystem=Subsystem.GENERAL,
        severity=Severity.ERROR,
        regex=re.compile(r"mmcblk\d+:\s*(?:error\b|timed out\b|I/O error\b|failed to send\b)", re.IGNORECASE),
    ...
    LogPattern(
        ...
        subsystem=Subsystem.GENERAL,
        severity=Severity.ERROR,
        regex=re.compile(r"NFS:.*(?:mount.*failed|No route to host|Connection refused)", re.IGNORECASE),
        extract_fields=["extra"],
