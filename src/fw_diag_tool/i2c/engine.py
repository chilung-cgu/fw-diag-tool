...
class I2CDiagnosticEngine:
        ...
        for ev in events:
            ...
            if ev.event_type in (RawEventType.START, RawEventType.REPEATED_START):
                if current_tx is not None:
                    if current_tx.byte_packets or current_tx.data_bytes or getattr(current_tx, "_has_address", False):
                        ...
                        if ev.event_type == RawEventType.REPEATED_START:
                            ...
                            current_tx.is_repeated_start = getattr(current_tx, "is_repeated_start", False)
                ...
                current_tx = I2CTransaction(
                    ...
                    address_8bit=((ev.address_7bit or 0x00) << 1) | (1 if ev.direction == I2CDirection.READ else 0),
            ...
            # ADDRESS Event
            if ev.event_type == RawEventType.ADDRESS or (ev.address_7bit is not None and ev.data_byte is None):
                addr_7b = ev.address_7bit or 0x00
                rw = ev.direction or I2CDirection.WRITE
                addr_8b = (addr_7b << 1) | (1 if rw == I2CDirection.READ else 0)
                
                is_placeholder = getattr(current_tx, "_is_placeholder", False) if current_tx else False
                packet_id_changed = (ev.packet_id is not None and current_tx is not None and ev.packet_id != getattr(current_tx, "_packet_id", None))
                ...
                if current_tx is None or (not is_placeholder and packet_id_changed):
                    if current_tx is not None and (current_tx.byte_packets or getattr(current_tx, "_has_address", False)):
                        current_tx.end_time = ev.timestamp
                        current_tx.duration_us = max(0.0, (current_tx.end_time - current_tx.start_time) * 1_000_000.0)
                        current_tx.has_stop = True  # packet_id boundary in Saleae represents clean packet framing
                        transactions.append(current_tx)
    ...
    def decode_semantic_layer(self, transactions: list[I2CTransaction]) -> None:
        ...
        for tx in transactions:
            ...
            ctx = device_context.setdefault(addr, {"last_cmd": None, "last_offset": None, "vout_exp": self.default_vout_exponent})
            ...
            if tx.protocol == "PMBus":
                if tx.direction == I2CDirection.WRITE:
                    if tx.data_bytes:
                        ...
                        decoded = decode_pmbus_payload(cmd_code, payload, vout_exponent=ctx["vout_exp"])
            ...
            # 2. EEPROM Protocol Semantic Decoding
                ...
                if tx.direction == I2CDirection.WRITE:
                    ...
                    decoded = decode_eeprom_read(tx.data_bytes, last_known_offset=ctx.get("last_offset"))
                    tx.semantic_summary = decoded.get("summary")
                    tx.decoded_values = decoded
            ...
            # 3. LM75 / TMP102 / Temperature Sensors
                ...
                if tx.direction == I2CDirection.WRITE:
                    if tx.data_bytes:
                        ptr = tx.data_bytes[0]
                        ctx["last_cmd"] = ptr
                        ptr_names = {0x00: "TEMP_REG", 0x01: "CONFIG_REG", 0x02: "THYST_REG", 0x03: "TOS_REG"}
                    ...
                    if ptr == 0x00:
                        ...
                        tx.semantic_summary = f"Read Register 0x{ptr:02X}: " + " ".join(f"0x{b:02X}" for b in tx.data_bytes)
