...
def analyze_timing_statistics(
    ...
    for tx in transactions:
        total_active_time_s += (tx.duration_us / 1_000_000.0)
    ...
    if all_inter_byte_delays_us:
        stats.avg_inter_byte_delay_us = sum(all_inter_byte_delays_us) / len(all_inter_byte_delays_us)
        stats.max_inter_byte_delay_us = max(all_inter_byte_delays_us)
        
    # Compute inter-transaction delay stats
    if all_inter_tx_delays_ms:
        stats.avg_inter_transaction_delay_ms = sum(all_inter_tx_delays_ms) / len(all_inter_tx_delays_ms)
        stats.max_inter_transaction_delay_ms = max(all_inter_tx_delays_ms)
        
    # Compute bus utilization
    if total_trace_duration_s > 0:
        stats.bus_utilization_pct = min(100.0, (total_active_time_s / total_trace_duration_s) * 100.0)
