    ...
    def process_transactions(self, transactions: list[I2CTransaction]) -> list[I2CDiagnosticIssue]:
        ...
        for tx in transactions:
            ...
            if addr in MUX_ADDRESSES:
                data_acked = (
                    all(p.ack == AckType.ACK for p in tx.byte_packets if not p.is_address)
                    if tx.byte_packets
                    else True
                )
                if (
            ...
            if self.last_active_mux is not None:
                ...
                if active_channels:
                    ...
                    tx.mux_topology = (
                        f"[MUX 0x{self.last_active_mux:02X}: {ch_str}] -> Slave 0x{addr:02X}"
                    )
                    tx.mux_channels = active_channels
                else:
                    tx.mux_topology = (
                        f"[MUX 0x{self.last_active_mux:02X}: ALL_OFF] -> Slave 0x{addr:02X}"
                    )
                    tx.mux_channels = []
