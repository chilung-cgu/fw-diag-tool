...
class PCIeReporter:
    ...
    def to_markdown(cfg: PCIeConfigSpace) -> str:
        ...
        lines.append(
            f"- **Class Code**: `0x{cfg.base_class:02X}{cfg.sub_class:02X}{cfg.prog_if:02X}` ({cfg.class_name})"
        )
        lines.append(f"- **Revision ID**: `0x{cfg.revision_id:02X}`")
        lines.append(
            f"- **Header Type**: `{cfg.header_type.name}` (Multi-Function: `{cfg.is_multi_function}`)"
        )
        lines.append(
            f"- **Command Register**: `0x{cfg.command:04X}` (MSE: `{bool(cfg.command & 0x02)}`, BME: `{bool(cfg.command & 0x04)}`, IOSE: `{bool(cfg.command & 0x01)}`)"
        )
        lines.append(
            f"- **Status Register**: `0x{cfg.status:04X}` (CapList: `{bool(cfg.status & 0x10)}`, MasterDataParity: `{bool(cfg.status & 0x8000)}`)"
        )
        lines.append("")
        ...
        if cfg.link_info:
            ...
            lines.append(
                f"- **Maximum Capable**: `{cfg.link_info.max_speed_str} x{cfg.link_info.max_width}`"
            )
            lines.append(
                f"- **Negotiated Status**: `{cfg.link_info.current_speed_str} x{cfg.link_info.current_width}`"
            )
            if cfg.link_info.is_degraded:
                lines.append(
                    f"- **Link Health**: `🚨 DEGRADED` ({cfg.link_info.degradation_reason})"
                )
                lines.append(f"\n```text\n{cfg.link_info.root_cause_guide}\n```\n")
            else:
                lines.append(
                    "- **Link Health**: `✔ OPTIMAL` (Operating at maximum designed capability)"
                )
            lines.append("")
        ...
        if cfg.header_type == HeaderType.TYPE_0_ENDPOINT:
            ...
            lines.append(
                f"- **Primary / Secondary / Subordinate Bus**: `{b.primary_bus}` / `{b.secondary_bus}` / `{b.subordinate_bus}`"
            )
            lines.append(f"- **Memory Window**: `0x{b.mem_base:08X}` - `0x{b.mem_limit:08X}`")
            lines.append(
                f"- **Prefetchable Memory Window**: `0x{b.pref_mem_base:08X}` - `0x{b.pref_mem_limit:08X}`"
            )
        ...
        if cfg.standard_capabilities:
            ...
            for cap in cfg.standard_capabilities:
                ...
                lines.append(
                    f"| `0x{cap.offset:02X}` | `0x{cap.cap_id:02X}` | {cap.name} | `0x{cap.next_offset:02X}` | {info_summary} |"
                )
        else:
        ...
        if cfg.extended_capabilities:
            ...
            for ext in cfg.extended_capabilities:
                lines.append(
                    f"| `0x{ext.offset:03X}` | `0x{ext.ext_cap_id:04X}` | v{ext.version} | {ext.name} | `0x{ext.next_offset:03X}` |"
                )
        else:
        ...
        if cfg.aer_analysis:
            ...
            lines.append(
                f"- **Uncorrectable Error Status / Mask / Severity**: `0x{aer.uncorr_status_raw:08X}` / `0x{aer.uncorr_mask_raw:08X}` / `0x{aer.uncorr_severity_raw:08X}`"
            )
            lines.append(
                f"- **Correctable Error Status / Mask**: `0x{aer.corr_status_raw:08X}` / `0x{aer.corr_mask_raw:08X}`"
            )
            lines.append(
                f"- **Active Uncorrectable Errors**: Fatal: `{aer.active_uncorr_fatal_count}`, Non-Fatal: `{aer.active_uncorr_nonfatal_count}`"
            )
            ...
            if active_uncorr:
                ...
                for err in active_uncorr:
                    ...
                    lines.append(
                        f"#### [{err.severity}] {err.name} (Bit {err.bit_pos}){masked_tag}"
                    )
            ...
            if aer.decoded_tlp:
                ...
                lines.append(
                    f"- **Traffic Class (TC)**: `{tlp.tc}`, **Digest (TD)**: `{tlp.td}`, **Poisoned (EP)**: `{tlp.ep}`"
                )
                ...
                if tlp.completer_id is not None:
                    lines.append(
                        f"- **Completer ID**: `0x{tlp.completer_id:04X}`, **Completion Status**: `{tlp.completion_status}`"
                    )
                lines.append("")
        else:
    ...
    def format_dmesg_events(events: list[DmesgAEREvent]) -> str:
        ...
        for idx, ev in enumerate(events, 1):
            ...
            lines.append(
                f"## Event {idx}: {ts_str}Device {ev.bdf} - {ev.error_name} ({ev.severity})"
            )
