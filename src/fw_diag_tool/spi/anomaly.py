"""SPI Flash Anomaly Detector and Root Cause Advisor."""
                    ...
                    issues.append(
                        SPIDiagnosticIssue(
                            ...
                            description=(
                                ...
                                "Write Enable (0x06 WREN) command or after WEL was cleared."
                            ),
                            root_cause_guide="""【Root Cause 排查建議】
1. NOR Flash 硬體保護機制：執行任何 Page Program, Erase 或 Status Register Write 之前，必須先發送單獨的 0x06 (WREN) 封包並拉高 CS#。
2. 檢查驅動代碼是否遺漏了 spi_flash_write_enable()。
3. 注意：每次 Program 或 Erase 完成後，Flash 硬體會自動將 WEL (Write Enable Latch) 歸零，下一次寫入必須重新發送 0x06！""",
...
2. 當寫入位址跨越 Page 邊界時，位址指標會直接 Wrap-around 回當前 Page 的 0x00 offset，導致該 Page 開頭的資料被非預期覆蓋，且後續 Page 完全沒有被寫入！
3. 修正方案：韌體中封裝 Page Write 驅動時，計算 chunk = min(length, 256 - (addr & 0xFF))，跨 Page 時必須拆分為兩次獨立的 Page Program 指令。""",
