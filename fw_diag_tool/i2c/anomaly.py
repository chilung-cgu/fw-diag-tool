    ...
    def analyze_transactions(
        ...
        for i, tx in enumerate(transactions):
            ...
            if tx.address_ack == AckType.NACK:
                ...
                prev_was_write = (i > 0 and transactions[i-1].direction == I2CDirection.WRITE and transactions[i-1].address_7bit == tx.address_7bit)
                ...
                if not pkt.is_address and pkt.ack == AckType.NACK:
                    if tx.direction == I2CDirection.READ:
                        ...
                        issues.append(I2CDiagnosticIssue(
                            ...
                            actionable_advice=[
                                "【暫存器檢查】查閱晶片手冊確認 0x" + f"{pkt.byte_val:02X}" + " 是否為合法的可寫入暫存器。",
            ...
            if not tx.has_stop and not tx.is_repeated_start and (i == len(transactions) - 1 or transactions[i+1].is_repeated_start is False):
                issues.append(I2CDiagnosticIssue(
            ...
            if tx.decoded_values and tx.decoded_values.get("rollover_hazard"):
                issues.append(I2CDiagnosticIssue(
                    ...
                    transaction_id=tx.id,
                    address_7bit=tx.address_7bit,
                    description=tx.decoded_values.get("rollover_details", "Page write exceeded page size boundary!"),
                    root_cause_analysis=(
        ...
        if timing_stats.frequency_jitter_pct > self.high_jitter_threshold_pct and timing_stats.avg_frequency_khz > 0:
            issues.append(I2CDiagnosticIssue(
